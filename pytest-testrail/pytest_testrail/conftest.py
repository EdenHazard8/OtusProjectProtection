...
def pytest_addoption(parser):
    ...
    group.addoption(
        '--tr-update',
        action='store_true',
        help='Update testplan or testrun')
    group.addoption(
        '--tr-merge_parametrize',
        action='store_true',
        help='Merge parametrize in tests')
    group.addoption(
        '--tr-testplan-name',
        action='store',
        default=None,
        help='Name given to testplan, that appears in TestRail (config file: testplan_name in TESTRUN section)')
    group.addoption(
        '--tr-publish_errors',
        action='store_true',
        help='Publish any errors')
    group.addoption(
        '--tr-custom_fields',
        action='store',
        default=None,
        required=False,
        help='Custom fields, example of writing - field1:item1,field2:item2'
    )
    ...
    if config.getoption('--testrail'):
        ...
        config.pluginmanager.register(
            PyTestRailPlugin(
                ...
                update=config_manager.getoption('tr-update', 'update', 'TESTRUN'),
                merge_parametrize=config_manager.getoption('tr-merge_parametrize', 'merge_parametrize', 'TESTCASE'),
                testplan_name=config_manager.getoption('tr-testplan-name', 'testplan_name', 'TESTRUN'),
                publish_errors=config_manager.getoption('tr-publish_errors', 'publish_errors', 'TESTCASE'),
                custom_fields=config_manager.getoption('tr-custom_fields', 'custom_fields', 'TESTCASE'),
            ),
