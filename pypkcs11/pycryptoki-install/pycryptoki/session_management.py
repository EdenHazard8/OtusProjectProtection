...
from .cryptoki import (CK_ULONG,
                       ...
                       CK_TOKEN_INFO,
                       CK_VOID_PTR,
                       CK_BYTE, CK_INFO, C_GetInfo, CA_GetFirmwareVersion,
                       CK_C_INITIALIZE_ARGS, CK_C_INITIALIZE_ARGS_PTR, c_ulong,
...
def jc_pki_unlock_user_pin(slot):
    """Unbock locked user pin

    :param slot: The slot to do a factory reset on
    :returns: The result code

    """
    LOG.info("JC_PKI_UnlockUserPIN: Unbock locked user pin. slot=%s", slot)
    ret = JC_PKI_UnlockUserPIN(CK_SLOT_ID(slot))
    return ret


jc_pki_unlock_user_pin_ex = make_error_handle_function(jc_pki_unlock_user_pin)
...
def c_get_function_list():
    """
    Returns a pointer to a list of pointers to an all functions of Cryptoki lib

    Can be called before C_Initialize()

    :return: pycryptoki.cryptoki.CK_FUNCTION_LIST object
    """

    functions_list = CK_FUNCTION_LIST_PTR()
    ret = C_GetFunctionList(byref(functions_list))

    return ret, functions_list


c_get_function_list_ex = make_error_handle_function(c_get_function_list)


def jc_kt2_set_signature_pin(slot, password):
    """Set signature pin for KT2. Needs user autentification

    :param slot: The slot to set pin on
    :param password: The user's desired password
    :returns: The result code
    """
    password = AutoCArray(data=password)
    ret = JC_KT2_SetSignaturePIN(CK_SLOT_ID(slot), password.array,
                                 password.size.contents)
    return ret


jc_kt2_set_signature_pin_ex = make_error_handle_function(jc_kt2_set_signature_pin)


def jc_kt2_change_signature_pin(slot, old_password, new_password):
    """Change signature pin for KT2. Needs user autentification

    :param slot: The slot to change pin on
    :param password: The user's desired password
    :returns: The result code
    """
    old_password = AutoCArray(data=old_password)
    new_password = AutoCArray(data=new_password)

    ret = JC_KT2_ChangeSignaturePIN(CK_SLOT_ID(slot),
                                    old_password.array,
                                    old_password.size.contents,
                                    new_password.array,
                                    new_password.size.contents)
    return ret


jc_kt2_change_signature_pin_ex = make_error_handle_function(jc_kt2_change_signature_pin)
